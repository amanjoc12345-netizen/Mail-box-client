...
const Sentbox = ({ emails = [], loading = false, error = '', onRefresh, onCompose }) => {
  ...
  // Handle deleting an email from Firebase sentbox
  const handleDeleteMail = async (mailId) => {
    if (!window.confirm('Are you sure you want to permanently delete this email from your sent records?')) {
      return;
    }
  ...
  const filteredEmails = emails.filter((mail) => {
    ...
    const receiver = (mail.receiver || '').toLowerCase();
    const subject = (mail.subject || '').toLowerCase();
    return receiver.includes(term) || subject.includes(term) || bodyText.includes(term);
  });
      ...
      {emails.length === 0 && !loading ? (
        <Card className="border-0 shadow-sm rounded-4 text-center py-5">
          <Card.Body className="py-5">
            <FiSend size={56} className="text-muted mb-3" style={{ opacity: 0.6 }} />
            <h3 className="fw-bold mb-2" style={{ color: '#1e293b' }}>No Sent Emails</h3>
            <p className="text-muted mx-auto mb-4" style={{ maxWidth: '380px' }}>
              Emails you compose and send will be stored here.
            </p>
            <Button
                ...
                {filteredEmails.length === 0 ? (
                  ...
                  filteredEmails.map((mail) => (
                    ...
                    >
                      ...
                      >
                        {mail.receiver ? mail.receiver.charAt(0).toUpperCase() : '?'}
                      </div>
                      ...
                      <div className="flex-grow-1 overflow-hidden">
                        <div className="d-flex justify-content-between align-items-baseline mb-1">
                          <span 
                            className="text-truncate text-secondary fs-6 fw-medium"
                        ...
                        <h6 className="text-truncate mb-1 text-dark" style={{ fontSize: '0.9rem' }}>
                      ...
                      <div className="d-flex align-items-center mt-3 gap-2">
                        ...
                        <div>
                          <div className="fw-semibold text-secondary" style={{ fontSize: '0.9rem' }}>
                            To: <span className="text-dark">{selectedMail.receiver}</span>
                          </div>
